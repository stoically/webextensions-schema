...
export class DownloadParse {
  ...
  private readonly outDir = join(__dirname, '..', '.schemas');
  ...
  set tag(tag: string) {
    this._tag = tag;
    this.tagDir = join(this.outDir, `${this.mozRepo}-${tag}`);
  }
  ...
  private async parseSchemas(): Promise<void> {
    ...
    await Promise.all(
      this.schemaTypes.map(async (type) => {
        const dir = join(this.tagDir, type, ...this.schemasDir);
        ...
        await Promise.all(
          files.map(async (file) => {
            ...
            const jsonBuffer = await fs.readFile(join(dir, file));
  ...
  private async downloadSchema(type: string): Promise<void> {
    ...
    await streamPipeline(response.body, Extract({ path: this.outDir }));
  }
